...
- [x] **P6-G6** Guide reader screen + in-app search across guides/tools *(`GuideReaderView` renders any `GuideContent` (title/summary, RDG note, sections, official-source links) with the `embedInScrollView` snapshot pattern. Home wrapped in a `NavigationStack` with a typed `HomeRoute` (`.guide`/`.search`); guide tiles push the reader. **Search:** pure `HomeSearch` engine (`items(for:)` = all guides + active mode's tools; `filter` = case/diacritic-insensitive title+subtitle match with injected `localize`) + `SearchView` (live field, browse/filter/no-results); guide hits open the reader, tool hits toast until their engines ship. Required moving `Guide`/`PersonaTool` title keys from `LocalizedStringKey` to `String` so search can resolve them (views wrap in `LocalizedStringKey`). `HomeSearchTests`/`SearchViewTests`/`GuideReaderViewTests` green; **214 tests**.)*
...
- [x] **P6-W3** **Chancenkarte points calculator** *(Pure `ChancenkarteEngine` sums criterion points and compares to the threshold; full recognition of a qualification short-circuits to "qualifies directly". Point values ported from the prototype and isolated in `ChancenkartePoints.current` (2026, threshold **6**: German A2/B1/B2 +1/+2/+3, English C1/native +1, age <35/35–39 +2/+1, experience 2–4y/5y+ +2/+3, shortage occupation +1, previous stay +1, partner applying +1 — **flagged for annual re-verification, OQ-1**); engine takes injected points so the rule is value-independent. `ChancenkarteView`: recognition toggle, per-criterion option rows / yes-no toggles with points badges, running points meter + qualifies/short-by-N verdict, residence-permit "learn more", RDG note, `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination`. 15 engine tests (each criterion, threshold boundary at exactly 6, direct path, learn-more guide exists) + 3 render tests; full suite green.)*
- [x] **P6-W4** Bank account comparison (affiliate) *(First affiliate/comparison tool — establishes a reusable `ComparisonOption`/`ComparisonCatalog` model (id, verbatim brand name, summary/highlight/fee/best-for localization keys, optional https URL, `isAffiliate`, `englishSupport`) for the later comparison tools (travel insurance, blocked account, student health, GKV/PKV). Versioned, factual `BankCompareCatalog.current` of 5 real Girokonto options ranked newcomer-first — **N26, DKB, ING, Commerzbank, Deutsche Bank** (English support, free-account conditions, smartphone vs. branch onboarding; **flagged for periodic re-verification, OQ-1**). Pure `BankCompareEngine.rankedOptions` keeps catalog order and applies an optional English-support filter, but never below the **P6-A1 3+-option invariant** (filter ignored if it would). `BankCompareView`: intro, prominent **affiliate transparency disclosure** (some links may earn a commission — never affects ranking/price), RDG note, English-only filter toggle, ranked cards (rank, brand, affiliate tag, summary, best-for, fee, highlights, `Link` open button), `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination`. 8 engine tests (≥3 options, unique ids, all URLs https, affiliate links present, ranking order, 3+ invariant unfiltered/filtered, filter keep/ignore) + 2 render tests; full suite green.)*
- [x] **P6-W5** GKV vs PKV decision tree (affiliate) *(Pure `HealthDecisionEngine` maps (insurance status, gross income) → a neutral, informational recommendation: employee at/below the compulsory-insurance threshold → `mustBeGKV`; employee above it, or self-employed / civil servant → `gkvOrPkvChoice`; student → `studentGKV`; other → `dependsOnCase`. The yearly **Jahresarbeitsentgeltgrenze** is isolated in `HealthInsuranceThreshold.current` (2026: €73,800 — **flagged for annual re-verification, OQ-1**); engine takes it injected so the rule is value-independent. Reuses the P6-W4 `ComparisonOption`/`ComparisonCatalog` for a versioned, factual `HealthInsurerCatalog` of 5 real funds ranked newcomer-first — **TK, AOK, Barmer, DAK-Gesundheit and the private (PKV) expat option Ottonova** (English support flagged where true; **re-verify periodically, OQ-1**). `HealthDecisionView`: status selector, income slider (employees only) with threshold line, recommendation card with neutral GKV vs PKV trade-offs, `"health_insurance"` guide "learn more", **affiliate transparency disclosure**, RDG note, ranked insurer cards, `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination`. 15 engine tests (each status, threshold boundary, every status produces a result, learn-more guide exists, ≥3 unique-id https affiliate options) + 2 render tests; full suite green.)*
- [x] **P6-W6** Tax-ID & Steuernummer organizer *(Pure, A-05-tested `TaxIdValidator`: `isValidSteuerID` strips spaces/slashes, requires exactly 11 digits with a non-zero leading digit, then applies the **official Steuer-ID check-digit algorithm** (the running product/modulo-11 loop, check = (11 − product) % 10) — rejecting wrong length, non-digits, leading-zero and any single-digit mutation; plus a deliberately loose `looksLikeSteuernummer` (digits + optional `/`/spaces, 10–13 digits) since the Steuernummer format is Bundesland-specific. Versioned, keys-only `TaxIdentifierCatalog`: 3 identifiers (**Steuer-ID** — 11 digits, lifelong, auto by post after Anmeldung; **Steuernummer** — per-activity, via ELSTER/Finanzamt; **USt-IdNr** — VAT ID for businesses), each with what/where/when, and a 5-step organizer checklist (Anmeldung → wait for the Steuer-ID letter → give it to your employer → request a Steuernummer if self-employed → keep the numbers safe), rendered as static `ChecklistRow`s (not wired to the persisted store). `TaxIdOrganizerView`: intro + RDG note, per-identifier explainer cards, the checklist, and a **live Steuer-ID validator field** (valid ✓ / not valid / empty) whose typed value lives in **session-only `@State` and is never persisted — sensitive PII, D4 privacy**, with an explicit "checked on-device, not stored" note; `"how_taxes_work"` guide "learn more"; `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination`. 16 validator/content tests (valid number built from the algorithm, spaces/slashes accepted, length/non-digit/leading-zero/bad-checksum rejected, loose Steuernummer accept/reject, ≥2 unique identifiers, ≥3 unique checklist steps, learn-more guide ships) + 2 render tests; full suite green.)*
- [x] **P6-W7** Freelance registration sub-flow *(Pure `FreelanceRegistrationEngine.steps(for:)` selects the ordered step list for a chosen legal form (`FreelancePath` — `freiberufler` / `gewerbe`). Versioned, keys-only `FreelanceRegistrationCatalog`: **Freiberufler** (confirm catalogue-profession status → file the *Fragebogen zur steuerlichen Erfassung* via ELSTER → receive Steuernummer → Kleinunternehmer vs VAT → arrange own health & pension/KSK insurance → start invoicing) and **Gewerbe** (Gewerbeanmeldung at the Gewerbeamt → Finanzamt questionnaire / ELSTER → automatic IHK/Handwerkskammer membership → receive Steuernummer → VAT + Gewerbesteuer above the allowance → arrange insurance). `FreelanceRegistrationView`: intro + RDG note, Freiberufler/Gewerbe path selector (defaults to Freiberufler), numbered step cards, an amber non-EU residence-permit caveat note, and a `"register_business"` guide "learn more"; `embedInScrollView` snapshot pattern. **Reachability:** added a 7th Worker tool tile (`freelance_registration`) and repointed the worker checklist `freelance_registration` item from the guide to `.tool("freelance_registration")`; registered in `implementedToolIDs` + `toolDestination`. 8 engine tests (both paths non-empty, unique ids, path heads, Gewerbe-only Gewerbeanmeldung/chamber steps, paths differ, ELSTER step, learn-more guide ships) + 2 render tests; PersonaCatalog deep-link/tile invariants still green; **full suite 309 tests**. **All Worker tools (P6-W1…W7) complete.**)*

**Tourist tools:**
- [x] **P6-T1** Visa-need decision tool *(Pure `VisaNeedEngine.result(for:data:)` normalizes an ISO 3166-1 alpha-2 nationality code (trim+uppercase) and classifies it via injected, versioned `VisaNeedData.current`: **free movement** (EU/EEA/Switzerland — no visa, may also live/work), **visa-free short stay** (EU "Annex II" exemption list), or **visa required** (Annex I = everything else; default for unknown codes). Memberships isolated as keys-only code sets — **re-verify against Regulation (EU) 2018/1806 Annexes I/II + the Auswärtiges Amt, OQ-1**; free-movement set = 31 (27 EU + IS/LI/NO + CH counted separately). Country **names come from `Locale`** (no country strings in the catalog). `VisaNeedView`: intro + RDG note, menu `Picker` of region codes sorted by localized name (built from `Locale.Region.isoRegions`), color-coded result card (primary / info / urgent) with a localized rationale carrying the country name; visa-free adds the **90-in-180 + ETIAS** note, visa-required adds the **verify-with-Auswärtiges-Amt** note; `"residence_permit"` "stay longer than 90 days?" footnote; `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination` (tile id `visa_need`). 9 engine tests (EU/EEA/CH free movement, Annex II visa-free, Annex I incl. TR visa-required, case-insensitive/trimmed, unknown/garbage → required, set size 31, visa-free non-empty/unique/well-formed, sets disjoint) + 3 render tests; **full suite 321 tests**.)*
- [x] **P6-T2** Embassy document checklist *(Pure `EmbassyChecklistEngine.documents(for:)` assembles, per `VisaPurpose` (`shortStay`/`work`/`study`/`family`), an ordered list = **common documents** (valid passport, completed & signed application form, biometric photos, travel/health insurance, proof of sufficient funds, appointment confirmation, visa fee) **+ purpose-specific** ones, all isolated as keys-only `ChecklistDocument`s (stable id + title + optional hint) in the versioned `EmbassyChecklistCatalog` — **requirements vary by country/consulate, re-verify against the Auswärtiges Amt / local German mission, OQ-1**. Purpose-specific: shortStay = travel itinerary/flight & accommodation booking + invitation/Verpflichtungserklärung; work = signed employment contract/job offer + qualification/Anerkennung + CV; study = university admission (Zulassung) + funding (Sperrkonto); family = marriage/birth certificate (legalised/apostilled) + sponsor lives in Germany + A1 German for spouses. `EmbassyChecklistView`: intro + RDG note (varies by consulate — always check the local mission's site), purpose selector (option rows, defaults to shortStay so a list shows immediately), the checklist as **interactive checkable "packing-list" rows** whose ticked ids live in **session-only `@State` and are never persisted** (by-design comment), and a `"residence_permit"` "learn more" for the long-stay purposes; `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination` (tile id `embassy_checklist`). 11 engine tests (every purpose non-empty, common docs in all purposes & first/in-order, ids unique per purpose, each purpose's signature doc present & exclusive, long-stay flag, learn-more guide ships) + 2 render tests; **full suite 334 tests**.)*
- [x] **P6-T3** Verpflichtungserklärung explainer *(Keys-only, versioned `VerpflichtungCatalog` exposed via a thin, tested `VerpflichtungContent` accessor: 6 ordered explainer sections (each id + heading + ≥1 body block) — **what it is** (a legally binding promise to cover the visitor's accommodation/living costs and, if needed, healthcare & return travel), **who needs it** (often required for short-stay Schengen visitor visas to prove funds), **where & how** (sponsor applies in person at the local Ausländerbehörde/Bürgeramt), **what the sponsor must prove** (sufficient, stable income — payslips/employment proof, sometimes a solvency check above a needs threshold), **cost & validity** (≈€25–40 fee, limited validity — re-verify, OQ-1), and **legally binding** (real financial liability) — plus a 4-item "what the sponsor brings" checklist (valid ID/passport, proof of income, rental contract/housing proof, the visitor's details). `VerpflichtungView`: intro, standard RDG `DisclaimerNote()` **and** a `severityLegal`-tinted "legally binding — get advice before signing" caution callout, explainer section cards, and the checklist as **session-only checkable packing-list rows** (held in `@State`, deliberately not persisted). `embedInScrollView` snapshot flag; no guide link. Registered in `implementedToolIDs` + `toolDestination` (tile id `verpflichtungserklaerung`, no args). 2 content tests (sections non-empty/unique-id/≥1 body each, checklist non-empty/unique-id) + 1 render test; **full suite 337 tests**.)*
- [x] **P6-T4** Travel insurance comparison (affiliate) *(Reuses the `ComparisonOption`/`ComparisonCatalog` model from P6-W4 (DRY) — no bespoke type. Versioned, factual `TravelInsuranceCatalog.current` of 5 real Schengen/visa travel-medical-insurance providers ranked newcomer-first — **HanseMerkur, DR-WALTER (Educare24), Care Concept, MAWISTA, Allianz Travel** (Schengen-visa-compliant ≥€30,000 cover, instant online certificate, English support, length-of-stay coverage; `monthlyFeeKey` repurposed as a neutral cover/price line; **flagged for periodic re-verification, OQ-1**). Pure `TravelInsuranceEngine.rankedOptions` mirrors `BankCompareEngine`: keeps catalog order, optional English-support filter never below the **P6-A1 3+-option invariant** (filter ignored if it would). `TravelInsuranceView` mirrors `BankCompareView`: intro noting the €30,000 Schengen requirement, dedicated Schengen-requirement callout, prominent **affiliate transparency disclosure**, RDG note, English-only filter toggle, ranked cards (rank, brand, affiliate tag, summary, best-for, cover, highlights, `Link` open button), `embedInScrollView` snapshot pattern. Registered in `implementedToolIDs` + `toolDestination` (`travel_insurance`). 8 engine tests (≥3 options, unique ids, all URLs https, affiliate links present, ranking order, 3+ invariant unfiltered/filtered, filter keep/ignore) + 2 render tests; full suite green.)*
