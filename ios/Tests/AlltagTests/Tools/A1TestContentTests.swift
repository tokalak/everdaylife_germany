...
/// Tests for the A1 German test guide content (P6-F2).
///
...
/// booking checklist is non-empty with unique ids. All keys must be non-empty.
/// The `residence_permit` guide the "learn more" link targets must ship in
/// `GuideLibrary`.
final class A1TestContentTests: XCTestCase {

    func testSectionsPopulated() {
        let sections = A1TestContent.sections
